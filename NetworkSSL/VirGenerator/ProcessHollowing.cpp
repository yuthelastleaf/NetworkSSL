...
int RunPE()
{
	...
	const LPVOID hFileContent = GetFileContent(IDR_BINARY_FILE);
	...
	SI.cb = sizeof(SI);
	GetStartupInfoA(&SI);  // 填充当前进程的启动信息

	PROCESS_INFORMATION PI;
	PI.hProcess = GetCurrentProcess();           // 当前进程句柄
	PI.hThread = GetCurrentThread();             // 当前线程句柄
	PI.dwProcessId = GetCurrentProcessId();      // 当前进程ID
	PI.dwThreadId = GetCurrentThreadId();        // 当前线程ID
